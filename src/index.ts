...
export function normalizeDiffs(value: unknown): DiffRecord[] {
...
function groupDiffsByPath(diffs: readonly DiffRecord[]): Map<string, DiffRecord[]> {
  const grouped = new Map<string, DiffRecord[]>();
...
function sameDiffs(left: readonly DiffRecord[], right: readonly DiffRecord[]): boolean {
  return (
...
export function diffLineCounts(oldText: string | null, newText: string): LineCounts {
...
function myersOperations(before: readonly string[], after: readonly string[]): DiffOperation[] {
...
export function buildUnifiedDiff(oldText: string | null, newText: string, contextLines = 3): UnifiedDiff {
  ...
  let added = 0;
  let removed = 0;
  const rows: UnifiedRow[] = operations.map((operation) => {
    if (operation.kind === "context") {
      const row: UnifiedRow = { kind: "context", oldLine, newLine, text: operation.text };
      oldLine += 1;
      newLine += 1;
      return row;
    }
    if (operation.kind === "del") {
      const row: UnifiedRow = { kind: "del", oldLine, newLine: null, text: operation.text };
      oldLine += 1;
      removed += 1;
      return row;
    }
    const row: UnifiedRow = { kind: "add", oldLine: null, newLine, text: operation.text };
    newLine += 1;
    added += 1;
    return row;
  });
...
export function summarizeDiffs(diffs: readonly DiffRecord[]): DiffStats {
  let added = 0;
  let removed = 0;
  for (const diff of diffs) {
...
function recordKey(callId: string, path: string): string {
  return `${callId}\u0000${path}`;
}
...
function validStoredRecord(value: unknown): value is Record<string, unknown> & {
  key: string;
  callId: string;
  turn: number;
  path: string;
...
function upsertRecord(state: SessionState, next: LedgerRecord): boolean {
...
function toolResultCallId(event: { data?: { message?: { content?: unknown } } | null }): string | undefined {
...
function scanSessionInto(state: SessionState, rootTurns: Map<string, { turn: number; step: number }>, session: LedgerSession): void {
...
export type SnapshotChainResult =
  | { ok: true; records: LedgerRecord[]; before: string | null; after: string }
  | { ok: false; reason: string };

export function resolveSnapshotChain(
  ...
  const chain: LedgerRecord[] = [finalCandidates[0]];
...
function compareRecordOrder(left: LedgerRecord, right: LedgerRecord): number {
  return (
...
function aggregateRecords(records: readonly LedgerRecord[]): FileReview[] {
  const files = new Map<string, { path: string; diffs: DiffRecord[]; records: LedgerRecord[] }>();
...
export interface ReviewStateSource {
  sessionId: string;
  revision: number;
  records: (LedgerRecord & { sourceSessionId?: string })[];
}

export function buildReviewState(state: ReviewStateSource, running = false): ReviewState {
  const grouped = new Map<number, LedgerRecord[]>();
  for (const record of state.records) {
    const records = grouped.get(record.turn) ?? [];
    records.push(record);
    grouped.set(record.turn, records);
  }
  const turns: TurnReview[] = [...grouped.entries()]
...
export interface FsService {
  ...
  stat(target: unknown): Promise<{ type: string; version: string; size: number } | undefined>;
  ...
  ): Promise<{ version: string; operation?: string; before?: string | null; after?: string }>;
...
export interface LedgerContext {
  ...
  fs: FsService;
  webServer: {
    register(options: { kind: string; path: string; handler: (req: IncomingMessage, res: ServerResponse) => Promise<void> }, label?: string): unknown;
  };
  sandboxPolicy: {
...
export function createLedgerRuntime(ctx: LedgerContext, storageRoot: string = STORAGE_ROOT): LedgerRuntime {
  const states = new Map<string, SessionState>();
  const rootTurns = new Map<string, { turn: number; step: number }>();
  ...
  const persistedValue = (state: SessionState): Record<string, unknown> => ({
  ...
  const schedulePersist = (state: SessionState): Promise<void> => {
  ...
  const latestSessionTurn = (session: LedgerSession): number | null => {
  ...
  const linkSession = (session: LedgerSession): SessionState => {
  ...
  const scanSession = (session: LedgerSession): void => {
    scanSessionInto(linkSession(session), rootTurns, session);
  };

  const ownerStates = (sessionId: string): SessionState[] =>
    [...states.values()].filter(
      (state) =>
        state.sessionId === sessionId || state.ownerSessionId === sessionId,
    );

  const projectedOwnerState = (sessionId: string): ReviewStateSource => {
  ...
  const ownerRunning = (sessionId: string): boolean => {
  ...
  const observeToolResult = (exec: ToolExec, result: ToolResult): void => {
        ...
        : [
            {
              ...
              oldText: value.before === null ? null : clippedText(value.before as string),
              newText: clippedText(value.after as string),
            },
          ];
    const state = linkSession(exec.agent.session);
    const changed = upsertRecord(state, {
      key: recordKey(exec.callId ?? "", value.path as string),
      callId: exec.callId ?? "",
  ...
  const observeSessionEvent = (session: LedgerSession, event: SessionEvent): void => {
  ...
  const reviewState = (sessionId: string): ReviewState => {
  ...
  const undoTurn = async (sessionId: string, turn: number): Promise<ReviewState> => {
    ensureState(sessionId);
    ...
    const prepared: {
      path: string;
      target: unknown;
      info: { type: string; version: string; size: number };
      current: string;
      baseline: string | null;
      changes: LedgerRecord[];
    }[] = [];
...
function routeHandler(runtime: LedgerRuntime): (req: IncomingMessage, res: ServerResponse) => Promise<void> {
  return async (req, res) => {
    try {
      ...
      if (req.method === "POST") {
        ...
        const state = await runtime.undoTurn(body.sessionId, body.turn as number);
...
function configRouteHandler(scope: SettingsScope): (req: IncomingMessage, res: ServerResponse) => Promise<void> {
  return async (req, res) => {
    try {
      if (req.method === "GET") {
...
export async function apply(ctx: LedgerContext, config: CodeReviewConfig = {}): Promise<void> {
  ...
  ctx.on("agent/created", ({ agent }: { agent: LedgerAgent }) => {
    runtime.scanSession(agent.session);
  });
  ctx.on("session/event", (session: LedgerSession, event: SessionEvent) => {
  ...
  ctx.on("session/flush", (session: LedgerSession) => runtime.flush(session.id));
  ctx.effect(
    () =>
